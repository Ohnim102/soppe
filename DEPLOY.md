...
```bash
docker run --rm -v "%cd%:/src:ro" node:20.20.2 bash -lc "mkdir -p /tmp/app && cp /src/package.json /src/package-lock.json /tmp/app/ && cp -a /src/frontend /tmp/app/frontend && cp -a /src/backend /tmp/app/backend && cd /tmp/app && npm ci --include=dev && npm run build"
```
